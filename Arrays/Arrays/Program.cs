...
static class Program
{
    ...
    //Creates a method. The method uses the parametres 'days', 'seasons' and 'year' to create a sentance 
    ...
    //Creates a method to generate city streets. It starts on a grid using X and Y co-ordinates, then goes on
    //to one of four directions. 0=right, 1=down 2=left 3=up.
    static void cityGen(bool[,] roads, int startX, int startY, int direction)
    {

        roads = new bool[startX, startY];
    }
    ...
    static void Main(string[] args)
    {
        ...
        //Attempt to make the program cycle through each value of 'i' until it it hits a boolean.
        //IF this boolean returns true, the program should continue to check the rest of the value along the Y axis until it runs into
        //another boolean value. It should then Console.Write a ','
        //ELSE, the program just outputs a '#'
        ...
        //Creates a loop with an initial counter of 20.
        //That loop will repeat the below operation for as long as the counter is larger than zero.
        //'i' will decrease with every repetition of the operation.
        ...
        for (int i = 20; i > 0; i--)
        {
            ...
            //Sets X=0 & Y=0 as parametres required to create a 'true' condition.

        roads[0, 0] = true;

            //This initializes 'road' as a boolean value.
            //The value is set to x=0 and y=1.

        bool road = roads[0, 1];

            //If the parametres required for a true condition are the intialized values,
            //the code below will write a ".".
        if ((road == true))
        ...
        //If the initialized values do not correspond to the condition required for true,
        ...
        //The console creates a loop that runs for as long as 'n' is larger than 0. 'n' is set to 20. It decreases with each rendition of the loop.

            for (int n = 20; n > 0; n--)
                //This loop outputs a new line and a 'T' for every iteration of the loop.
                Console.WriteLine("T");

            //Creates a loop that runs for as long as b is smaller than 20. B is initially set to '0' and increases with one for every iteration.

            for (int b = 0; b < 20; b++)
            {
                //For as long as this loop runs, the console will write '#'
                Console.Write("#");
            }
        }
...
class Program
{
    ...
    static void Main(string[] args)
    {
        GenerateIntersection(roads, 4, 2);
        DrawRoads();
    }
    ...
    static void GenerateIntersection(bool[,] roads, int x, int y)
    {
        ...
        /* //Creates a string of integers using the directions previously outlined.
         int[] directions = { 0, 1, 2, 3 };

         //Sets the initial value of 'checkDirection' to 0.
         int checkDirection = 0;

         //A do loop that loops while the 'while' condition is met.
         do
         {
             //This loop loops through the string of integers named 'directions'
             foreach (int i in directions)
             {
                 //While doing so, it outputs a random number for each interger, betwee 1-100.
                 checkDirection = random.Next(1, 100);

                 //Checks if the outputted number is smaller than, or equal to, 71.
                 if (checkDirection <= 71)

                     //if the above condition is true, the method returns the specific integer than the loop stopped at.
                 { return i;}

             }
             //This loop repeats if the random number is above 71.
         } while (checkDirection > 71);

         //Returns '1' if something else happens. What, I do not know. But the computer complains and refuses to run the code otherwise. I think it had a bad day or something.
         { return 1; }
    ...
    static void GenerateRoad(bool[,] roads, int startX, int startY, int direction)
    {
        //When the 'startX' & 'startY' value for Roads is true
        ...
        //A loop is created. The index valiue is set to 'y'. This loop will continue for as long as 'height' is bigger than 'y'.
